...
from numpy import empty, genfromtxt
import pandas as pd 
import os 
import numpy as np
import serial
...
# configFileName = 'AWR1642_SDK2.01.cfg'
    ...
    while True:
        ...
        if magicOK:
            ...
            try:
                ...
                if next_frame == 1:
                    FrameIndex += 1
                    print(f"populating......{FrameIndex}")

            except Exception:
                traceback.print_exc()
            
            # print(FrameIndex)
            print(f'-----------------FRAMEID{FrameIndex}-------------------')
            # print(GestureData) 
            print('-----------------DONE-------------------')
            if FrameIndex >= num_of_frames: 
                # if slope == 1: 
                slope = 0
                FrameIndex = 0 
                # print(GestureData)
                dirpath = 'inference_' + str(GestureNumber) + '.csv'
                np.savetxt(dirpath, GestureData, delimiter=',', fmt=['%d', '%d', '%f', '%f', '%f', '%f', '%f'],
                        header='timestamp, ObjectNumber,Range,Velocity,PeakValue,x,y', comments='')    
                GestureData = np.array([(0, 0, 0, 0, 0, 0, 0)], dtype=dtype)
                GestureNumber = 1
                read_file_for_inference()


def read_file_for_inference():
    print("Start reading data from csv file")
    label = ["right","stop"]
    path = "inference_1.csv"
    i = 3
    # path3 =  f"data/stop_gesture/gesture_{i}.csv"
    # path4 =  f"data/arm_to_right/gesture_{i}.csv"
    ...
    output = np.argmax(model(data_result.reshape(1,-1)))
    pred = label[output]
    # os.remove("inference_1.csv")
    print("-----REMOVE INFERENCE------")
    print(pred)
...
def configureSerialAWR():
    ...
    serial_port_1 = ports1
    serial_port_2 = ports2
...
def save_data(GestureData):
    np.savetxt('data/Gesture.csv', GestureData, delimiter=',', fmt=['%d', '%d', '%f', '%f', '%f', '%f'],
               header='timestamp, ObjectNumber,Range,Velocity,x,y', comments='')
